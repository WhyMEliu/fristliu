...
var main, allRow, grade, stopFlag = false, bgMusic;
	...
	timer = setInterval(
			function() {
				...
				if (flag) {
					 ...
					 * 
					 * 
					 * for(var i = 0; i < allRow.length; i++) { var row =
					 * allRow[i].getElementsByTagName('div'); for(var j = 0; j <
					 * 
					 * row; j++) { row[j].rowPos = i; } }
					 */
				}
			}, tSpeed);
}
...
function keyPlay(event) {
	...
	if (event.keyCode == 113) { // 用户按下F2，开始游戏
		...
		stopGame();
	} else if (event.keyCode == 68 || event.keyCode == 70 ||
	...
	event.keyCode == 74 || event.keyCode == 75) {
		if (!stopFlag) {
			...
			blackGridPos = -1; // blackRowPos 该行黑格的位置
			for ( var i = 0; i < allRow.length; i++) {
			...
			blackGridPos != -1) {
				...
				|| (event.keyCode == 70 && blackGridPos == 1)
					 ...
					 * 
					 * (bgMusic.play) { bgMusic.pause(); } }, 3000);
					 */
...
function rightChange(blackRowPos, blackGridPos) {
	...
	[blackRowPos].hasBlackGrid = false;
...
function mousePlay() {
	for ( var i = 0; i < allRow.length; i++) {
		...
		allRow[i].getElementsByTagName('div');
		for ( var j = 0; j < row.length; j++) {
...
function isGameOver(obj) {
	console.debug("obj.dffsetTop===距离顶端变化的高度=="+obj.offsetTop+"  obj.offsetHeight==130=="+obj.offsetHeight);
	var temp1 = obj.offsetTop + obj.offsetHeight;
	console.debug("main.offsetTop==0=="+main.offsetTop+"  main.offsetHeight==560==="+main.offsetHeight);
	var temp2 = main.offsetTop + main.offsetHeight - 20;

	/*console.debug("temp1--obj:"+temp1+"  temp2--main:"+temp2);*/
	if (temp1 > temp2) {
		if (obj.hasBlackGrid) {
            console.debug("--------------"+obj.hasBlackGrid)
